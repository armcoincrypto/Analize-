...
class PatchGenerator:
    ...
    def generate_patch(
        ...
        # Save patch file
        patch_filename = f"patch_{request.request_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.patch"
...
"""
        ...
        patch_filename = f"env_patch_{request.request_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.patch"
        patch_path = self.patches_dir / patch_filename
...
"""
        ...
        script_filename = f"apply_{request.request_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.sh"
